...
const MovieCard = ({movie}) => {
  ...
  return (
    <div className='flex flex-col justify-between p-3 bg-gray-800 rounded-2xl hover:-translate-y-1 transition duration-300 w-56'>
        ...
        src= {movie.backdrop_path} alt="" className='rounded-lg h-52 w-full object-cover object-right-buttom cursor-pointer'/>
 ...
 className='px-4 py-2 text-xs  bg-red-600 hover:bg-red-700 transition rounded-full font-medium cursor-pointer'>Buy Tickets</button>
        <p className='flex items-center gap-1 text-sm text-gray-400 mt-1 pr-1'>
            <StarIcon className='w-4 h-4 inline-block text-primary fill-primary'/> {movie.vote_average.toFixed(1)}
        </p>
    </div>
    </div>
  )
