...
useEffect (()=>{
  ...
  duration-300`} onClick={() => setSelectedMovie(movie.id)}>
            <div className='relative rounded-lg overflow-hidden'>
              ...
              <div className='text-sm flex items-center justify-between p-2 bg-black/70 w-full absolute bottom left-0'>
              <p className='flex items-center gap-1 text-gray-400'>
                <StarIcon className='w-4 h-4 text-red-600'/></p>
                <p className='text-gray-300'>{kConverter(movie.vote_count)}Votes</p></div>
            </div>
    ...
    <div className='mt-6'>
      <label className='block text-sm font-medium mb-2'> Select Date and Time     </label>
        <div className='inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg'>
          ...
          <button onClick={handleDateTimeAdd} className='bg-red-600/80 text-white px-3 py-2 text-sm rounded lg hover:bg-red-600 cursor-pointer'> Add Time</button>
        </div>
    </div>
    ...
    {Object.keys(dateTimeSelection).length > 0 && (
      <div className='mt-6'>
        <h2 className='mb-2'>Selected Data-Time</h2>
          ...
          {Object.entries(dateTimeSelection).map(([date,times]) => (
            <li key= {date}>
              ...
              <div className='flex flex-wrap gap-2 mt-1 text-sm'>
                {times.map((time) => (
                  <div key={time} className='border border-red px-2 py-1 flex items-center rounded'>
                    <span>{time}</span>
